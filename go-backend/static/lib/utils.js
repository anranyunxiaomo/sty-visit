...
const VisitUtils = {
    ...
    async protectPwd(pwd, sessionKey) {
        if (!pwd) return '';
        if (pwd.startsWith('PROTECTED:AES:')) return pwd;
        if (!sessionKey) return pwd;
        try {
            ...
            return 'PROTECTED:AES:' + btoa(String.fromCharCode(...iv)) + ':' + btoa(String.fromCharCode(...new Uint8Array(encrypted)));
        } catch (e) { return pwd; }
    },
    ...
    async revealPwd(protectedStr, sessionKey) {
        if (!protectedStr) return '';
        
        if (protectedStr.startsWith('PROTECTED:AES:')) {
        ...
        if (protectedStr.includes(':') && sessionKey) {
            try {
        ...
        try {
            return atob(protectedStr);
        } catch (e) {
            return protectedStr; // raw text fallback
        }
    }
};
